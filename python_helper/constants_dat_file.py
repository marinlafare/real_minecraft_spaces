...
def create_template_datfile(title="some_data_idk"):
    """
    Creates a basic NBT template for a Minecraft map data file.
    Includes crucial xCenter and zCenter tags, initialized to 0.
    These will be updated with correct values when saving actual map data.

    Args:
        title (str): The title for the NBT root.

    Returns:
        nbtlib.File: An nbtlib File object representing the map data structure.
    """
    root = nbtlib.File()
    root.name = title
...
def create_maps(image_path, world_name, channels=3):
    ...
    # Assuming 'saves' is a sibling directory to where the script is run
    ...
    for ind, map_file_data in enumerate(maps_colors):
        ...
        # Calculate xCenter and zCenter for this specific map tile
        # Maps are centered at (X*128 + 64, Z*128 + 64) for scale 0
        x_center = col * 128 + 64
        z_center = row * 128 + 64
        ...
        # Path for Minecraft world save directory
        world_data_path = os.path.join(parent, f'saves/{world_name}/data')
        # if not os.path.exists(world_data_path):
        #     os.makedirs(world_data_path) 
        output_filename_world = os.path.join(world_data_path, f'map_{n_file}.dat')
        nbtfile = create_template_datfile(title=f"{base_image_name}_map_{n_file}") # Title usually matches filename without .dat

        # Extract only the color IDs from map_file_data
        color_ids = [entry[0] for entry in map_file_data]
        map_byte_data = bytearray(color_ids) # Convert list of ints to bytearray

        # Update the NBT tags with the actual data
        ...
        try:
            nbtfile.save(output_filename_local) # Save to local 'maps' folder
            nbtfile.save(output_filename_world) # Save directly to world's data folder
            #print(f"Saved map_{n_file}.dat with xCenter={x_center}, zCenter={z_center}")
    ...
    if facing in 'west':
        mcfunction_lines.append(
                        fill_line(
                            ...
                            -1, 
                            num_rows_of_maps - 1, 
                            num_cols_of_maps, # z2: Same depth
                            'dirt'
                            )
                        )
    if facing in 'east':
    ...
    if facing in ['up','down']:
        mcfunction_lines.append(
                        fill_line(
                            ...
                            -(num_rows_of_maps - 1), 
                            -1, 
                            num_cols_of_maps -1, # z2: Same depth
                            'dirt'
                            )
                        )
